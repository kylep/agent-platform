  ...
  return (
    <>
      <StatRow>
        ...
        <Stat label="new input/output tokens · last 5000 runs" value={`${m.tokens_in.toLocaleString()} / ${m.tokens_out.toLocaleString()}`} />
      </StatRow>
      <h2>Tokens by model <span className="muted text-sm font-normal">(all time; cached input shown in Reporting)</span></h2>
      <Table>
        <thead><tr><TH>Model</TH><TH>Runs</TH><TH>New input</TH><TH>Output</TH></tr></thead>
        <tbody>
          {models.map((mu) => (
            <tr key={mu.model}>
