    ...
    <div className="page">
      ...
      {ov && (
        <StatRow>
          ...
          <Stat label={`new input/output tokens · last ${ov.window} runs`} value={`${ov.tokens_in.toLocaleString()} / ${ov.tokens_out.toLocaleString()}`} />
      ...
      <p className="muted">Cache reuse is the share of input tokens served from cache, not the share of runs with a cache hit.</p>
      ...
      <Table>
        <thead><tr><TH>Model</TH><TH>Runs</TH><TH>New input</TH><TH>Output</TH><TH>Cached input</TH><TH>Cache reuse</TH></tr></thead>
        <tbody>
          {models.map((m) => (
            <tr key={m.model}>
              <TD>{m.model}</TD>
              ...
              <TD className="text-muted">{(m.tokens_cache_read + m.tokens_cache_creation + m.tokens_in) > 0
                ? `${Math.round(100 * m.tokens_cache_read / (m.tokens_cache_read + m.tokens_cache_creation + m.tokens_in))}%`
                : "—"}</TD>
            </tr>
          ))}
