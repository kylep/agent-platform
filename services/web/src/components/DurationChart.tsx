...
// Series colors come from the design system (--ds-chart-1..8) so both themes
// keep WCAG AA contrast for the legend text.
...
export default function DurationChart() {
  ...
  const color = (agent: string) => `var(--ds-chart-${(agents.indexOf(agent) % CHART_SERIES) + 1})`;
  ...
  return (
    <div>
      <div className="chip-row" style={{ marginBottom: 6 }}>
        {[7, 14, 30, 90].map((d) => (
          <ChipButton key={d} variant={d === days ? "ok" : "neutral"}
                      onClick={() => setDays(d)}>{d}d</ChipButton>
        ))}
        <ChipButton onClick={() => setLog(!log)}>
          {log ? "log" : "linear"} ⇄
        </ChipButton>
        <span style={{ width: 12 }} />
        {agents.map((a) => (
          <ChipButton key={a} style={{
                        opacity: hidden.has(a) ? 0.35 : 1,
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: "100%", maxWidth: W, display: "block" }}>
        ...
        {visible.map((p) => (
          <circle key={p.run_id}
                  ...
                  stroke={p.state === "succeeded" ? "none" : "var(--ds-danger)"}
