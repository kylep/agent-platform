...
async def test_memory_default_can_be_opted_out_or_disabled_platform_wide(admin_client, sf):
    r = await admin_client.post("/api/agents", json={**a_def("forgetful"), "memory": False})
    assert r.status_code == 201, r.text
    assert MEMORY not in await tools_of(sf, "forgetful")
    admin_client._transport.app.state.settings.memory_default_grant = False
    r = await admin_client.post("/api/agents", json=a_def("no-default"))
    assert r.status_code == 201, r.text
    assert MEMORY not in await tools_of(sf, "no-default")
    r = await admin_client.post("/api/agents", json={**a_def("explicit"), "memory": True})
    assert r.status_code == 201, r.text
    assert MEMORY in await tools_of(sf, "explicit")
