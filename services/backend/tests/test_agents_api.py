...
# `mcp__platform__relay`, `mcp__platform__tickets` and `mcp__platform__wiki`
# while the platform settings say so, and an admin can take any of them away
# afterwards like any other grant. A setting each, so "a messenger but no work
# tracker" is expressible.
...
async def test_create_grants_the_participant_tools_by_default(admin_client, sf):
    ...
    assert r.status_code == 201, r.text
    assert r.json()["platform_tools"] == [RELAY, TICKETS, WIKI]
    assert await tools_of(sf, "newbie") == [RELAY, TICKETS, WIKI]
...
async def test_create_may_opt_out_of_a_participant_grant(admin_client, sf):
    ...
    assert r.status_code == 201, r.text
    assert await tools_of(sf, "quiet") == [TICKETS, WIKI]
...
async def test_the_relay_default_can_be_turned_off_platform_wide(admin_client, sf):
    ...
    assert await tools_of(sf, "mute") == [TICKETS, WIKI]
    # ...and an explicit `relay: true` still asks for it.
    assert (await admin_client.post("/api/agents",
                                    json={**a_def("loud"), "relay": True})).status_code == 201
    assert await tools_of(sf, "loud") == [RELAY, TICKETS, WIKI]
...
async def test_the_relay_default_is_not_the_caller_escalating(client, sf, seed_agent,
    ...
    assert r.status_code == 201, r.text
    assert await tools_of(sf, "ordinary") == [RELAY, TICKETS, WIKI]
...
async def test_import_grants_the_participant_tools_and_stays_idempotent(admin_client, sf):
    ...
    assert await tools_of(sf, "fresh") == [RELAY, TICKETS, WIKI]
