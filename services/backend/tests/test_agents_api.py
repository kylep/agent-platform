...
# --- the participant default grants (docs/design/19, 20, 21) ------------------
...
# `mcp__platform__get_quota_usage` and `mcp__platform__artifacts` while the
# platform settings say so, and an admin can take any of them away afterwards
# like any other grant. A setting each, so "a messenger but no work tracker" is
# expressible.

RELAY = "mcp__platform__relay"
TICKETS = "mcp__platform__tickets"
WIKI = "mcp__platform__wiki"
QUOTA = "mcp__platform__get_quota_usage"
ARTIFACTS = "mcp__platform__artifacts"
...
async def test_create_grants_the_participant_tools_by_default(admin_client, sf):
    ...
    assert r.status_code == 201, r.text
    assert r.json()["platform_tools"] == [RELAY, TICKETS, WIKI, QUOTA, ARTIFACTS]
    assert await tools_of(sf, "newbie") == [RELAY, TICKETS, WIKI, QUOTA, ARTIFACTS]
    ...
    assert (await versions_of(sf, "newbie"))[0].snapshot["platform_tools"] == [
        RELAY, TICKETS, WIKI, QUOTA, ARTIFACTS]
...
async def test_the_relay_default_can_be_turned_off_platform_wide(admin_client, sf):
    ...
    assert await tools_of(sf, "mute") == [TICKETS, WIKI, QUOTA, ARTIFACTS]
    # ...and an explicit `relay: true` still asks for it.
    assert (await admin_client.post("/api/agents",
                                    json={**a_def("loud"), "relay": True})).status_code == 201
    assert await tools_of(sf, "loud") == [RELAY, TICKETS, WIKI, QUOTA, ARTIFACTS]
...
async def test_the_relay_default_is_not_the_caller_escalating(client, sf, seed_agent,
    ...
    assert r.status_code == 201, r.text
    assert await tools_of(sf, "ordinary") == [RELAY, TICKETS, WIKI, QUOTA, ARTIFACTS]
...
async def test_import_grants_the_participant_tools_and_stays_idempotent(admin_client, sf):
    payload = [a_def("fresh"), {**a_def("quiet"), "relay": False, "tickets": False,
                                "wiki": False, "get_quota_usage": False,
                                "artifacts": False}]
    r = await admin_client.post("/api/agents/import", json=payload)
    assert r.json() == [{"name": "fresh", "status": "created"},
                        {"name": "quiet", "status": "created"}]
    assert await tools_of(sf, "fresh") == [RELAY, TICKETS, WIKI, QUOTA, ARTIFACTS]
