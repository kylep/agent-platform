...
# `tickets_grant=False`/`wiki_grant=False` throughout: these are design-19's
# sweep, and Tickets (docs/design/20) and the Wiki (docs/design/21) each run one
# of their own over the same rows under their own marks, with their own tests.
# Letting those run here would have every assertion below carry grants it is
# not about.
