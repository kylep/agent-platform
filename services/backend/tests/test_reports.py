...
def test_repo_registry_loads_shipped_types():
    ...
    assert dn.generator == "app:news" and dn.cadence == "daily" and dn.retention_days == 365
...
async def test_machine_keys_gated_by_declared_generator(client, sf):
    # daily-news declares `generator: app:news` — only the app's own key (by
    # principal name) may write it; agent run keys and other apps are refused.
    await client.post("/api/setup", json={"password": "pw12345678"})
    async def mint(name, agent):
        token = generate_token()
        async with sf() as s:
            s.add(ApiKey(name=name, role="operator", agent=agent,
    ...
    r = await client.post("/api/reports", json=body,
                          headers=await mint("invoke:news", "news"))
    assert r.status_code == 403 and "generated by" in r.json()["detail"]
    r = await client.post("/api/reports", json=body,
                          headers=await mint("app:other", None))
    assert r.status_code == 403
    r = await client.post("/api/reports", json=body,
                          headers=await mint("app:news", None))
