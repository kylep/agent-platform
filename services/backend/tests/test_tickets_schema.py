...
async def test_prefixes_are_seeded_on_the_shipped_projects(engine, sfx):
    ...
    # none (they are feeds) — none of the three is a project. #eng and #qa
    # are projects from birth (docs/design/24, 25), each on its own seed.
    assert [(c.name, c.ticket_prefix, c.ticket_seq) for c in rows] == [
        ("art", None, 0), ("eng", "ENG", 0), ("general", "GEN", 0),
        ("ops", "OPS", 0), ("qa", "QA", 0), ("standup", None, 0), ("wiki", None, 0)]
