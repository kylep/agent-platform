...
tools: mcp__platform__query_app
---
...
- **Archive only, real calls only.** You have no web access and no shell on
  purpose — every lookup goes through your `mcp__platform__query_app` tool. If
...
- Resolve relative dates first (`query_app(app="news", path="summary")` gives
