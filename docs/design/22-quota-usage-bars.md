...
| `POST /api/quota/refresh` | `READ_ROLES` + `relay` | Runs the probes through the provider proxies, observes the result (`source="refresh"`), and returns the same shape as GET. `provider=codex` or `provider=claude` selects one; the default is both. Coalesced: one in-flight probe per process; a caller arriving during one waits for its result. Each provider short-circuits to the cache when it was observed under `QUOTA_REFRESH_MIN_SECONDS` (default 15) ago and is not stale. 503 with a plain `detail` when the selected required proxy is unreachable or returns no usable reading. |
